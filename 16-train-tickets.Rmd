...
library(stringr)
library(purrr)
library(lubridate)  ## Because I don't know how to make %within% exist otherwise
```
...
## As seen in https://dplyr.tidyverse.org/articles/colwise.html:
rowAny <- function(x) rowSums(x) > 0

bad.tickets <-
    tickets %>%
    filter(rowAny(across(everything(), ~ ! (.x %within% allowed.values))))
```

The error rate is `r count(bad.tickets) / count(tickets)`
